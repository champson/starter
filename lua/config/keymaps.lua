...
map("n", "<localleader>sP", LazyVim.telescope("live_grep"), { remap = true, desc = "Grep (Root Dir)" })
-- stylua: ignore
map("n", "<leader>n", "<cmd>Telescope buffers sort_mru=true sort_lastused=true<cr>", { remap = true, desc = "Switch Buffer" })
-- stylua: ignore
map("n", "<localleader>sp", LazyVim.telescope("grep_string", { word_match = "-w" }), { remap = true, desc = "Word (Root Dir)" })
...
map("n", "<C-p>", LazyVim.telescope("files"), { remap = true, desc = "Find Files (Root Dir)" })
